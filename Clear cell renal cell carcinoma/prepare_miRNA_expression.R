## PREPARE MIRNA EXPRESSION DATA FOR ccRCC DATSET
## ==============================================
...
## load GSE16441 miRNA expression data
fList <- list.files("data/raw/", full.names = TRUE)[35:68]
...
geo <- getGEO("GSE16441")
geo <- geo[[grep("GPL8659", names(geo))]]

## define samples metadata
meta <- pData(geo)
meta <- meta[, c(1, 2, 42, 43)]
...
## save metadata
write.csv(meta, "data/mirna_metadata.csv")
...
## save the plot
intPlot <- recordPlot()
saveRDS(intPlot, paste(plotsFolder, "miRNA_probe_intensities.RDS", sep = "/"))
...
saveRDS(exprFlt, "data/miRNA_expression_matrix.RDS")
